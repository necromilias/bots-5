...
This inventory applies to the uncommitted unified-grant candidate based on
`89fb52979c67b8b1c822a7efb3c5a67946611bdf`.  It is the finite sibling-path
audit required by the accepted architecture.  `DataRootAuthority` is the only
root admission and invalidation coordinator.  Store `_poisoned`, application
close state, helper `_closed` fields, native UNKNOWN generations and migration
journal states are diagnostics, resource evidence, or local lifecycle gates;
none independently grants forward work.

## Invalidation-route dispositions (22/22)

| Route | Disposition in this candidate |
|---|---|
...
| Effect | Disposition in this candidate |
|---|---|
...
| E13 generation provenance persistence | Both start paths retain transition ownership across context/source/message/attempt rows, attachment associations, CAS, transaction settlement and return reads. |
...
| E20 configuration/credentials | Every public `ProviderConfiguration` method acquires/joins command admission, including Secret Service put/delete and status reads. No credential/provider was accessed by this campaign. |
...
`Executable` names the direct oracle. `Preserved` refers to still-applicable
tests in `test_phase6_context_attachments.py` and the Phase 3–5 files.
...
| 19 | Preserved executable native open/delete/auxiliary/private-close fault matrices plus new thread-generation handoff and authority ledger source assertions. |
...
| 25 | Executable/source: complete `_SQLITE_OPERATION_METHODS` and `_PHASE5_OPERATION_METHODS` wrapper inventory, complete public configuration-facade wrapper inventory, private engine/pre-creator rejection, expired connection and private delivery rejection. The I21 corrupt-required-payload oracle proves callee invalidation and rejects a subsequent durable chat write. |
...
`tests/mutants/run_authority_effect_mutants.py` copies `src` into a fresh
temporary tree per variant and never rewrites the candidate.  Required result:
...
The audit searched direct terminal assignments, raw engine/DBAPI creation,
caller-only wrappers, stale/copied context, cleanup forward admission, detached
publication, native UNKNOWN propagation, phase-capability escape, parallel
poison flags and resource lifetimes longer than grants.  Two concrete gaps were
found and repaired during this audit: provisional logical-socket close outcome
accounting (I22), and composite shutdown fallback ownership (E17). Resource
sets were also added to terminal drain accounting, bound private event delivery
now requires an issued effect grant, and native thread attribution is keyed per
VFS rather than sharing a cross-VFS counter. Fresh Terra then demonstrated a
third gap: I21 canonical-payload verification was translated into a local
`StateError` without revoking authority. The canonical callee now invalidates,
and the reproduction plus a removal mutant are release-critical evidence. No
unresolved sibling escape is recorded in this revised candidate inventory;
fresh reviewers must attempt to falsify that conclusion.
